...
function onTripAdding(socket) {
	socket.on("tripAdding", function (listRequest) {
		onAddingTrip(socket, listRequest);
	});
}
...
function onEachUserConnection(socket) {
	...
	onLocationChangeDriver(socket);
}


function onAddingTrip(socket,listRequest){
	//
	print('tripDAta: ' + stringifyJson(listRequest));
	// Get the 'to' User...
	let to_user_id = listRequest.to;
	let from_user_id = listRequest.from;
	...
	if (!userOnline) {
		...
		listRequest.message_sent_status = STATUS_MESSAGE_NOT_SENT;
		listRequest.to_user_online_status = false;
		sendBackToClient(socket, SUB_EVENT_MESSAGE_FROM_SERVER, listRequest);
		return;
	}

	// User Connected and his Socket ID Found on the UserMap
	listRequest.message_sent_status = STATUS_MESSAGE_SENT;
	listRequest.to_user_online_status = true;
	sendToConnectedSocket(socket, to_user_socket_id, SUB_EVENT_RECEIVE_MESSAGE, listRequest);
...
function onLocationChangeDriver(socket){
	socket.on("position", function (position) {
	let lnt = position.lat;
	...
	// 	checkOnlineHandler(socket, chat_user_data);
	// });
	socket.broadcast.emit("driverLocation", position);
...
function checkOnlineHandler(socket, chat_user_data) {
	...
	print('Checking Online 	: ' + to_user_id);
